...
graph TD
    ...
    subgraph External ["外部整合服務"]
        ...
        ZPlanner["公司工時系統 (ZPlanner)<br/>https://zplanner.internal/workspace/"]
    end
...
3. **Action Items 智慧互動轉換**：
   ...
   - 支援一鍵複製工時與待辦格式，預留對接自研工時系統 `ZPlanner` (https://zplanner.internal/workspace/)。
4. **一鍵雙向回推**：
...
- **Should Have (第二階段)**：
  - 跨群組多選聯合週報產生器
  - Action Items 格式化匯出至公司自研工時系統 `ZPlanner` (https://zplanner.internal/workspace/)
