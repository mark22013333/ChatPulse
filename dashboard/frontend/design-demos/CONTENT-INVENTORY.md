...
🚫 **這一欄刻意不卡片化。**依據是規格 §5.2「版式：一本帳本」，它同時立了三件事：

1. 「數字自己形成一條乾淨的縱列——**這是整套設計最主要的識別特徵**」。
   切成卡片，所有列就不再共用同一個右緣。
2. 線型**本身承載語意**（`╌╌╌` 降級／`───` 完整／`═══` 小計，`--line-evidence` 受
   WCAG 1.4.11 管 ≥3:1）。卡片的裝飾邊框（`--line`）會與它混在一起，而規格 §3.2
   的整個重點就是「兩種規線是兩件事」。
3. 「缺值顯示 `—`，**不抽掉那一列**」「標籤永不變淡」——位置固定才分得出
   「沒有圖片」與「還沒讀到圖片」。卡片化會鼓勵人把空的那張拿掉。

§5.2 的 ASCII 示意圖畫的就是**整欄一個外框、內部用線型分隔**。
要讓它在版面上成為「一個獨立的面」，改整欄底色（`bg-surface`）就夠，不需要內部卡片。
