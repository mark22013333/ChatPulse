...
> 2026-09-06 改名——**取名的判準見下方「歸屬標示」，重點是不能取成人名**，
> 因為那是專案層級的共用設定。
>
...
- **歸屬標示拿不掉（2026-09-05 查證）**：透過 API 送出的訊息，Chat 會在發送者名字旁邊
  ...
  - **這個標籤與本決策的方向相反**：架構上一切以本人身分發話，UI 上卻掛著一個
    ...
    而不是「一隻機器人」的名字。
  - **⚠️ App name 是 GCP 專案層級的設定，對所有使用者是同一個字串。**
    這一點決定了它**不能取成任何人的名字**：團隊共用同一份 `client_secret.json`
    ＝同一個專案，所以取成 `MarkCheng` 會讓同事的訊息顯示成
    `王小明 [MarkCheng] 12:36`——別人的訊息旁邊掛著維護者的名字，
    比原本的 `T-Bot` 更容易誤導。
  - **命名判準：取工具名，不要取人名，也不要取像機器人的名字。**
    2026-09-06 的改名過程正好踩到這一點：先從 `T-Bot` 改成 `MarkCheng`
    （解決了「像機器人」），使用者隨即發現它對同事不成立（製造了「像某人」），
    最終定為工具名。記下這段是因為**這個錯誤只有在多人使用時才看得出來**，
    單人測試永遠不會暴露它。
