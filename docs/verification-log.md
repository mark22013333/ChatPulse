...
**2026-09-05 09:50 更新：這一級已經沒有項目了。**

原本卡著的是「483 則對話用 16384 跑出 `finishReason=STOP`」那一次。我先前判斷
「必須等太平洋午夜配額重置」——**那個判斷是錯的**。配額錯誤訊息裡的 quotaId 寫得
很清楚：`GenerateRequestsPerDayPerProjectPerModel-FreeTier`，**每個模型各有 20 次**。
我讀到那個字串卻沒去用它。`core/config.py` 早就有 `CHATPULSE_GEMINI_MODEL` 可以換模型。

換 `gemini-3.7-flash` 跑之後不只補齊了證據，還**推翻了原本對 D-3 根因的理解**——
見下方「模型差異」那一節。這是本次最該記的一課：**「做不到」有時只是沒把已知的
線索用上**，那句 `PerModel` 就寫在我自己貼進報告的錯誤訊息裡。

### D-3（`maxOutputTokens` 2048 vs 16384）——含模型差異

同一份 483 則對話（29,611 字對話文本、29,969 字 prompt），來自「PSTB上班沒壓力之群組」。

**跨模型比較（這一組推翻了原本的根因說法）**

| 模型 | maxOutputTokens | finishReason | thoughts | candidates | 輸出字數 | 章節 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| gemini-3.6-flash | 2048 | **MAX_TOKENS** | 1,966 | 78 | 118 | 0/3 |
| gemini-3.6-flash | 2048（另一次） | **MAX_TOKENS** | 1,962 | 82 | 115 | 0/3 |
| gemini-3.7-flash | 2048 | **STOP** | **0** | 1,269 | 1,774 | **3/3** |
| gemini-3.7-flash | 16384 | STOP | 1,820 | 1,259 | 1,787 | 3/3 |

**同一份 prompt、同樣的 2048 上限，3.6-flash 被截斷、3.7-flash 沒事。** 差別在
思考量：3.6-flash 花了近 2,000 個思考 token，3.7-flash 那一次花了 0。

所以 2048 的問題不是「一定不夠」，是**時好時壞、壞的時候安靜截斷**。
唯一恆成立的判準是「完整輸出所需的總預算（思考＋正文）> 2048」：
3.7-flash 是 3,079，3.6-flash 是 3,782，兩者都遠超過。

以下是 3.6-flash 早期兩輪的原始數字（保留供對照）：
