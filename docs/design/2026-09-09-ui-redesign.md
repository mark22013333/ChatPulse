...
> ⚠️ **2026-09-12 有後續改版**，見 [`2026-09-12-admin-console.md`](2026-09-12-admin-console.md)。
> 它**推翻了本文件的兩個決定**：§3.2「分群靠規線不靠填色」（淺色已改成卡片式分層，
> 深色維持原樣）、§17 紅線第 3 條對 `route.ts`／`AppShell`／`TopBar` 的限制（已解除，
> 改以「測試零減少零失敗」把關）。**其餘內容全部繼續有效**——尤其是 §10.8 的對比門檻、
> §7.2 的保留掛載手法、§10.2 的導覽無障礙要求，以及紅線的其他九條。
...
.dark {
  color-scheme: dark;
  --background:    oklch(0.222 0.010 250);  /* #181b20  底層 */
  --surface:       oklch(0.256 0.010 250);  /* #1f2428  ΔL +0.034 */
  --raised:        oklch(0.290 0.011 250);  /* #272c31  ΔL +0.034 對 surface */
  --foreground:    oklch(0.918 0.005 250);  /* #e1e4e7  13.53:1  刻意不到 16:1 */
  --fg-dim:        oklch(0.752 0.007 250);  /* #abafb3   7.82:1 */
  --fg-subtle:     oklch(0.672 0.007 250);  /* #93969a   5.81:1（raised 面 4.74） */
  --muted:         oklch(0.282 0.010 250);  /* #262a2e  ΔL +0.026 對 surface */
  /* 不透明而非 alpha：alpha 邊框在多層面上會互相疊加變深 */
  --line:          oklch(0.338 0.010 250);  /* #34383d   1.46:1  純裝飾 */
  --line-strong:   oklch(0.418 0.010 250);  /* #494d52   2.03:1  純裝飾 */
  --line-evidence: oklch(0.564 0.012 250);  /* #70767d   3.76:1  承載語意（raised 面 3.07） */

  --signal:        oklch(0.780 0.118 245);  /* #73bffe   8.72:1  色域內（該 L 上限 C 0.120） */
  --signal-on:     oklch(0.200 0.010 250);  /* 深字在 signal 上 9.17:1 */
  --signal-wash:   oklch(0.780 0.118 245 / 0.10);
  --signal-line:   oklch(0.780 0.118 245 / 0.34);
  --verified:      var(--signal);

  --caution:       oklch(0.800 0.072 78);   /* #d8b989   9.22:1 */
  --caution-line:  oklch(0.800 0.072 78 / 0.45);

  --provenance:    var(--fg-subtle);
  --destructive:   oklch(0.702 0.165 22);   /* #f46f6f   6.04:1 */
  --disabled-fg:   oklch(0.505 0.007 250);  /* #626569   2.95:1 */
  ...
  /* 暗色沒有光源：浮起＝環線 + 頂緣內高光，不是模糊灰影。
     ...
     **環線不用純黑**：純黑疊在任何底色上都會壓回近黑（實算 #060708），
     而吃到它的正好是命令面板、快捷鍵說明、證據抽屜——見下方 2026-09-10 修訂 */
  --elevation-raised:  0 0 0 1px oklch(0.145 0.008 250 / 0.55),
                       inset 0 1px 0 0 oklch(1 0 0 / 0.06);
  --elevation-overlay: 0 0 0 1px oklch(0.145 0.008 250 / 0.75),
                       inset 0 1px 0 0 oklch(1 0 0 / 0.07),
                       0 18px 40px -14px oklch(0.100 0.008 250 / 0.7);
}
...
> **2026-09-10 修訂：深色的地板抬離黑色。**
>
> 使用者實機回報「頁面不要做全黑」。量測之後有三個可指出的成因，
> **單獨抬背景只解掉其中一個**：
>
> 1. **地板就踩在黑上。** 舊 `--background` 是 `#121416`（L 0.189），離純黑
>    `#121212` 只差 0.009；深色主題的舒適帶大約 L 0.18–0.25，舊值在最底部。
> 2. **文字對比 16.19:1，離純黑白的 21:1 只差 23%。** halation（細筆畫發光
>    糊掉）是「前景亮度 ÷ 背景亮度」的函數，不是背景值單獨決定的——而這個
>    產品要讀的是密集中文與程式碼，筆畫多、最吃這一項。
> 3. **畫面上最暗的像素不是背景，是浮層的環線。** `--elevation-*` 的第一層
>    原本是字面純黑 `oklch(0 0 0 / …)`，疊在 `#121416` 上算出來是 `#060708`
>    ／`#08090a`，比背景還暗一階。吃到它的正好是命令面板、快捷鍵說明、
>    證據抽屜這三個最常開的浮層（抽屜還是常駐的）。
>
> **階層落差刻意不放大**（`background → raised` 總 ΔL 0.070 → 0.068）。
> 這一版的分群靠規線而不是填色（§3.2），把階層拉開等於改成卡片式——那是
> 重新設計。附帶好處：同樣的 ΔL 發生在較亮的位置時 WCAG 比值反而更大，
> 總落差 1.186 → **1.226**。
>
> **彩度 0.004 → 0.010 才是「不死板也不變藍」的旋鈕**（色相 250 不動，與淺色
> 共用）。實測：C 0.004 的冷調量 B−R 只有 3、實質等於中性灰；0.010 是 8；
> 到 0.028（B−R 23）就已經讀起來像藍色主題。0.010 只佔該亮度色域上限的 15%。
>
> **順手修掉三個既有就不達標的對比。** 舊註解只對 `--background` 算過一次，
> 沒對 `--surface`／`--muted`／`--raised` 算，所以漏了：
>
> | token | 最壞面 | 舊值 | 新值 | §10.8 門檻 |
> | :--- | :--- | ---: | ---: | ---: |
> | `--fg-subtle`（＝`--provenance`） | raised | **4.21** ✗ | 4.74 ✓ | 4.6 |
> | `--signal` | muted | **6.93** ✗ | 7.30 ✓ | 7 |
> | `--line-evidence` | raised | **2.82** ✗ | 3.07 ✓ | 3（WCAG 1.4.11） |
>
> 四個面逐一驗過的完整表（33 條全通過、全部在 sRGB 色域內）：
>
> | token | hex | on background | on surface | on muted | on raised |
> | :--- | :--- | ---: | ---: | ---: | ---: |
> | `--foreground` | #e1e4e7 | 13.53 | 12.27 | 11.32 | 11.04 |
> | `--fg-dim` | #abafb3 | 7.82 | 7.09 | 6.55 | 6.38 |
> | `--fg-subtle` | #93969a | 5.81 | 5.27 | 4.87 | 4.74 |
> | `--signal` | #73bffe | 8.72 | 7.91 | 7.30 | 7.12 |
> | `--caution` | #d8b989 | 9.22 | 8.37 | 7.72 | 7.53 |
> | `--destructive` | #f46f6f | 6.04 | 5.48 | 5.06 | 4.93 |
> | `--line-evidence` | #70767d | 3.76 | 3.41 | 3.15 | 3.07 |
> | `--disabled-fg` | #626569 | 2.95 | 2.67 | 2.47 | 2.40 |
>
> **`--signal` 的彩度是被色域逼著降的**（0.144 → 0.118）：要抬到 L 0.780 才能
> 在 muted 面守住 7:1，而 h 245 在該亮度的 sRGB 上限只有 C 0.120。這是「7:1」
> 與「藍色濃度」之間的真實取捨，選了規格。
>
> **`--signal-line`（2.15）與 `--caution-line`（2.90）仍低於 3:1**，但兩者都
> **比舊值高**（1.98／2.79），而且 §3.2 只把 `--line-evidence` 定為承載語意的
> 規線；這兩個是選中態與提示框的裝飾框線，且不是意義的唯一載體（選中態同時有
> `bg-signal-wash` ＋ `font-medium`，caution 框內本身就有 caution 色的文字）。
>
> **淺色主題一個字都沒動。** 兩個主題各自完整宣告同一組原始變數，
> `@theme inline` 裡只有 `var()` 指標——這正是它不需要改的原因。
>
> 順帶修掉一個與配色無關的小漂移：`--radius: 6px` 原本只宣告在 `:root`、
> `.dark` 沒有（規格這裡有、實作漏了）。今天不會出事——`.dark` 與 `:root`
> 落在同一個 `<html>` 上，會正確繼承——但理由與相容層那段一樣：`.dark`
> 若被搬到某個 wrapper div 就會壞。**已補進 `index.css` 的 `.dark`，
> 兩檔現在一致。**
