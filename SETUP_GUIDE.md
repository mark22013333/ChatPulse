...
### Q8：摘要和草稿到底讀了哪些訊息？討論串的回覆也會讀嗎？

**都會讀到討論串的回覆，但兩個功能的範圍差很多**，這會直接影響你看到的結果品質。

| | 摘要 | 產生草稿 |
| :--- | :--- | :--- |
| 讀取範圍 | 該群組**最近 N 則**訊息（預設 50，可調 1~1000） | 被 @ 的那則 ＋ **該討論串的全部訊息**（上限 1000） |
| 討論串回覆 | 會讀到，但**可能只是片段** | 完整 |
| 參考群組 | — | 每個勾選的群組各讀最近 N 則（行為同摘要） |

**「片段」是什麼意思**——摘要是**按時間**取最近 50 則，不是按討論串取。所以一個有
20 則回覆的討論串，如果散落在不同時間，摘要可能只讀到其中 3 則，AI 看到的是被切斷
的對話。這就是為什麼摘要偶爾會漏掉某個討論的結論。

**想讓摘要看到完整討論串**：把「抓取則數」調大。群組越熱鬧、討論串越長，需要的數字
越大。代價是讀越多、AI 的 token 花費越高，也越慢。

產生草稿沒有這個問題——它是先鎖定那個討論串，再把整串撈完，所以脈絡一定是完整的。

**附件與圖片**：只有在 AI 供應商支援看圖時才會下載，而且限制不少——只掃最近 30 則、
最多 8 張、單檔超過 12MB 跳過、只收直接上傳到 Chat 的 png/jpeg/gif/webp（Google Drive
的附件不會碰）。完全不想讓它讀圖的話，設環境變數 `CHATPULSE_IMAGES=0` 即可。
